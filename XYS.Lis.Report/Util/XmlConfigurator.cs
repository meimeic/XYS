...
namespace XYS.Lis.Report.Util
{
    public class XmlConfigurator
    {
        ...
        private static readonly string HANDLE_TYPE_ATTR = "handleType";
       ...
       // private static readonly string FILL_TAG_ATTR = "fillTag";
        ...
        private static void ConfigElement(XmlElement element)
        {
            if (element.LocalName == FILL_ELEMENT_TAG)
            {
                ...
                string handleName = element.GetAttribute(HANDLE_TYPE_ATTR);
                if (!string.IsNullOrEmpty(typeName)&&!string.IsNullOrEmpty(handleName))
                {
                    FillElement ele = new FillElement(name, typeName,handleName);
