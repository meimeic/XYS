...
namespace XYS.FR.Lis.Util
{
    public class ConfigManager
    {
        ...
        private static readonly LisItemMap ITEM_MAP = new LisItemMap(40);
        private static readonly LisGroupMap GROUP_MAP = new LisGroupMap(20);
        private static readonly LisPrintModelMap MODEL_MAP = new LisPrintModelMap(20);
        #endregion
        ...
        #region
        public static void AddGroup(LisGroup group)
        {
            GROUP_MAP.Add(group);
        }
        public static void AddLisItem(LisItem item)
        {
            ITEM_MAP.Add(item);
        }
        public static void AddPrintModel(LisPrintModel model)
        {
            MODEL_MAP.Add(model);
        }
        #endregion
        ...
        //{
        //    table.Clear();
        //    if (ITEM_MAP.Count == 0 || GROUP_MAP.Count == 0)
        //    {
        //        InternalConfigure();
        //    }
        ...
        //    foreach (object rs in ITEM_MAP.AllReporterSection)
        //    {
        ...
        private static void InternalConfigure()
        {
            ...
            //获取文件全名
            fullPath2ConfigFile = Path.Combine(applicationBaseDirectory, "XYS.FR.Lis.xml");
            try
            {
        ...
        private static void InternalConfigure(Stream configStream)
        {
            ...
            else
            {
                ...
                if (doc != null)
                {
                    ...
                    //获取节点集合
                    XmlNodeList configNodeList = doc.GetElementsByTagName("lisFR");
                    //节点必须有且只有一个
                    if (configNodeList.Count == 0)
                    {
                        ConsoleInfo.Error(declaringType, "XML configuration does not contain a <lisFR> element. Configuration Aborted.");
                    }
                    else if (configNodeList.Count > 1)
                    {
                        ConsoleInfo.Error(declaringType, "XML configuration contains [" + configNodeList.Count + "] <lisFR> elements. Only one is allowed. Configuration Aborted.");
                    }
                    else
                    {
                        //根据节点信息，配置repository
